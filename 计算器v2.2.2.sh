...
# 配置信息
REPO_OWNER="jiangyoung112"
REPO_NAME="shell-calculator"
VERSION_FILE="https://raw.gitmirror.com/${REPO_OWNER}/${REPO_NAME}/main/version.info"
UPDATE_SCRIPT="https://raw.gitmirror.com/${REPO_OWNER}/${REPO_NAME}/main/update.sh"
CURRENT_VERSION="2.2.2"
UPDATE_LOG="$HOME/.calculator_update.log"
...
check_update() {
    ...
    # 获取远程版本信息
    remote_info=$(curl -sL "$VERSION_FILE" || echo "error")
    ...
    remote_version=$(echo "$remote_info" | awk '/^VERSION=/ {print substr($0,9)}')
    changelog=$(echo "$remote_info" | sed -n '/^CHANGELOG=/,$p' | tail -n +2)
...
perform_update() {
    ...
    # 下载更新脚本
    if ! curl -sL "$UPDATE_SCRIPT" -o "$tmp_dir/update.sh"; then
        echo -e "${RED}更新文件下载失败，请手动从https://github.com/jiangyoung112/shell-calculator/tree/main?tab=readme-ov-file#下载最新更新${NC}"
        rm -rf "$tmp_dir"
        sleep 3
        return 1
    fi
    ...
    # 验证数字签名（可选）
    if ! verify_signature "$tmp_dir/update.sh"; then
    ...
    if "$tmp_dir/update.sh" "$REPO_OWNER" "$REPO_NAME"; then
        echo -e "\n${GREEN}更新成功！请重新启动程序${NC}"
        log_update "success" "$remote_version"
        rm -rf "$tmp_dir"
        sleep 3
        exit 0
    else
        echo -e "\n${RED}更新过程中发生错误${NC}"
        log_update "failed" "$remote_version"
        sleep 3
        rm -rf "$tmp_dir"
        return 1
    fi
...
verify_signature() {
    # 此处应实现实际签名验证逻辑
    # 例如使用GPG验证：
    # gpg --verify update.sig $1
    return 0 # 暂时跳过验证
}

# 更新日志记录
log_update() {
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] $1 - $2" >> "$UPDATE_LOG"
}

# 在主菜单前添加自动检查
auto_check_update() {
    # 每天只检查一次更新
    #last_check=$(date -r "$UPDATE_LOG" +%s 2>/dev/null)
    #current_time=$(date +%s)
...
show_changelog() {
    ...
    echo -e "${YELLOW}[v2.2.2] 2025-04-15${NC}"
    echo "  - 优化自动更新系统"
    echo -e "${YELLOW}[v2.2.1] 2025-04-14${NC}"
    echo "  - 添加自动更新系统"
    echo -e "${YELLOW}[v2.2] 2025-04-13${NC}"
    echo "  - 数学盒子升级至v2.2"
    echo "  - 新增质数验证功能"
    echo "  - 添加斐波那契数列生成器"
    echo "  - 集成经典数学谜题合集"
